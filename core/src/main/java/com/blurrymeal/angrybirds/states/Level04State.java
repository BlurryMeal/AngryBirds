...
public class Level04State extends State {
    ...
    private boolean isEditableMode = true; // Flag to indicate editable mode
    ...
    protected void handleInput() {
        if (Gdx.input.justTouched()) {
            ...
            if (isEditableMode) {
                if (Gdx.input.isButtonPressed(Input.Buttons.LEFT)) {
                    // Place vertical slab
                    obstacles.add(new Obstacles(new Texture("woodVertObst.png"), touchPos.x, touchPos.y, 7, 45, world, 0f));
                } else if (Gdx.input.isButtonPressed(Input.Buttons.RIGHT)) {
                    // Place horizontal slab
                    obstacles.add(new Obstacles(new Texture("woodVertObst.png"), touchPos.x, touchPos.y, 7, 45, world, 90f));
                }
            }
            ...
            if (touchPos.x >= 880 && touchPos.x <= 960 && touchPos.y >= 320 && touchPos.y <= 380) {
                if (isEditableMode) {
                    isEditableMode = false; // End editable mode
                } else {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) {
            ...
            // Add a new pig at the mouse position
            pigs.add(new Pigs(new Texture("smallPig.png"), mousePos.x, mousePos.y, 27, 27, world, 160f));
            pigCounter++;
        }
    }
